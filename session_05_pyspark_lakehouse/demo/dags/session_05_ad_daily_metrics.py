...
@dag(
    dag_id="session_05_ad_daily_metrics",
    description="Ad analytics: bronze CSV -> silver deduped fact -> gold daily metrics",
    schedule="@daily",
    ...
    tags=["session-05", "lakehouse", "spark", "delta", "ads"],
)
def session_05_ad_daily_metrics():
    ...
    build_fact_ad_events = _spark_submit(
        "bronze_2_silver__build_fact_ad_events", "/opt/airflow/jobs/demo/build_fact_ad_events.py"
    )
    aggregate_ad_daily = _spark_submit(
        "silver_2_gold__aggregate_ad_daily", "/opt/airflow/jobs/demo/aggregate_ad_daily.py"
    )
